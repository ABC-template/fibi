...
// Версия: 2.8.0 - ДОБАВЛЕН ПАРАМЕТР sync ДЛЯ ОБНОВЛЕНИЯ ТОКЕНА
// ============================================
...
class ChatService {
    ...
    async fullReload() {
        ...
        try {
            // ✅ ВСЕГДА ДЕЛАЕМ sync=true ПРИ ПОЛНОЙ ПЕРЕЗАГРУЗКЕ!
    ...
    // ==========================================
    // ✅ ЗАГРУЗКА ВСЕХ ЧАТОВ (С ПАРАМЕТРОМ sync)
    // ==========================================
    ...
    async loadAllChats(options = {}) {
        ...
        try {
            ...
            // ✅ Если sync=true → добавляем параметр в URL
            ...
            if (data.success && data.chats) {
                ...
                // ✅ Если вернулся syncToken — сохраняем
                if (data.syncToken) {
                    this._saveSyncToken(data.syncToken);
                }
    ...
    async loadMetadata(silent = false) {
        if (!this.userStore.canSync()) {
            console.log('⏭️ Синхронизация отключена');
            return;
        }
    ...
    mergeChatFromCloud(cloudChat, topicId) {
        ...
        if (found) {
            ...
            const newChat = this.chatStore.createChat(
                ...
                cloudChat.title,
                {
                    ...
                    synced: true,
                    messages: cloudChat.messages.map(m => ({
                        id: m.id,
                        text: m.text,
    ...
    async permanentDeleteChat(chatId) {
    ...
    async uploadLocalDataToCloud() {
            ...
            try {
                ...
                if (action === 'Повторить') {
                    ...
                    if (window.uiRenderer) {
                        window.uiRenderer.showToast('❌ Загрузка отменена. Данные откатаны.', 'error', 3000);
                    }
                    return false;
                }
            }
        ...
        if (errors.length > 0) {
            if (window.uiRenderer) {
                window.uiRenderer.showToast(`⚠️ Загружено ${uploaded} из ${total} чатов. ${errors.length} пропущено.`, 'warning', 3000);
            }
        } else {
            if (window.uiRenderer) {
                window.uiRenderer.showToast(`✅ Все ${total} чатов успешно загружены в облако!`, 'success', 3000);
            }
        }
    ...
    async updateChatInCloud(localChat) {
    ...
    async createChatInCloud(localChat) {
    ...
    async rollbackUpload(uploadedCount, localChats) {
    ...
    hideProgressModal() {
        ...
        this._uploadStartTime = null;
        this._uploadTotal = 0;
    }

    showErrorDialog({ message, error, options }) {
...
console.log('✅ ChatService v2.8.0 загружен (добавлен sync=true)');
