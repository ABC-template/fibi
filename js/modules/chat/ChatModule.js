...
class ChatModule {
    ...
    _showWelcomeMessage(container) {
        const topic = this._topic || 'code';
        const welcomeText = window.welcomeTexts?.[topic] || 
            'Привет! Я Versatile AI. Чем могу помочь?';
        
        const welcomeDiv = document.createElement('div');
        welcomeDiv.className = 'msg ai-msg welcome-msg';
        welcomeDiv.id = 'welcome-message';
        
        const contentContainer = document.createElement('div');
        contentContainer.style.width = '100%';
        contentContainer.textContent = welcomeText;
        
        welcomeDiv.appendChild(contentContainer);
            ...
            },
            {
                ...
                onClick: () => {
                    if (this.eventBus) {
                        ...
                        window.showContextModal(this._chatId);
                    }
                }
            },
            {
    ...
    _initFooter() {
        ...
        if (mediaBtn) {
            mediaBtn.addEventListener('click', () => {
                if (window.triggerMediaSelector) {
        ...
        if (voiceBtn) {
            voiceBtn.addEventListener('click', () => {
                if (window.toggleVoiceRecording) {
                    window.toggleVoiceRecording(voiceBtn);
                }
            });
        }
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', () => {
                if (window.clearUserText) {
                    window.clearUserText();
                }
            });
        }
    }
