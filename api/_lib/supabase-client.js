...
// Версия: 11.0.0 - ИСПРАВЛЕНА ЛОГИКА ПОИСКА ПОЛЬЗОВАТЕЛЯ
// ============================================
 ...
 */
export async function getUserUuid(telegramId, config = null) {
    try {
        ...
        console.error('Failed to get user UUID:', err.message);
        return null;
    }
}

/**
 * ✅ НОВОЕ: Обновить sync_token пользователя
 */
export async function updateSyncToken(telegramId, config = null) {
    try {
        ...
        console.error('Failed to update sync token:', err.message);
        return null;
    }
}

/**
 * ✅ НОВОЕ: Получить текущий sync_token пользователя
 */
export async function getSyncToken(telegramId, config = null) {
    try {
        const result = await supabaseFetch(
...
// ==========================================
// ОСТАЛЬНЫЕ ФУНКЦИИ (проверка лимитов, canUserSync и т.д.)
// ==========================================
...
// ✅ ВЕРСИЯ 11.0.0 - ИСПРАВЛЕНА ЛОГИКА ПОИСКА
// ==========================================

export async function getOrCreateAuthUser(telegramId, userData, config = null) {
    ...
    try {
        ...
        console.error(`❌ [getOrCreateAuthUser] Ошибка поиска в public.users:`, err.message);
    }
    ...
    if (existingUser) {
        ...
        try {
            ...
            if (userById.ok) {
                ...
                // ✅ НАШЛИ! Ничего не создаём, просто используем
            } else if (userById.status === 404) {
                // ❌ КРИТИЧЕСКАЯ ОШИБКА: данные разъехались!
                ...
                console.error(`❌❌❌ Это нарушает целостность данных! Нужно срочно разобраться!`);
            ...
            if (err.message.includes('Целостность данных нарушена')) {
                throw err;
            }
            console.error(`❌ [getOrCreateAuthUser] Исключение при поиске в auth.users:`, err.message);
            throw new Error(`Ошибка при поиске в auth.users: ${err.message}`);
        }
    }
    ...
    if (!existingUser) {
        ...
        try {
            ...
            console.log(`ℹ️ [getOrCreateAuthUser] Ошибка поиска в auth.users:`, err.message);
        }
    }
    ...
    if (authUser && !existingUser) {
        ...
        isNew = false; // Пользователь уже существовал в auth
    ...
    if (!userPassword) {
        ...
        try {
            ...
            console.warn(`⚠️ [getOrCreateAuthUser] Не удалось обновить пароль в auth.users:`, err.message);
        }
    }
    ...
    try {
        ...
        if (signInResponse.ok) {
            ...
            try {
                ...
                console.warn(`⚠️ [getOrCreateAuthUser] fallback ошибка:`, fallbackErr.message);
            }
        }
    } catch (err) {
        console.warn(`⚠️ [getOrCreateAuthUser] sign-in ошибка:`, err.message);
    }
...
// ==========================================
// ВСПОМОГАТЕЛЬНАЯ: ПОЛУЧИТЬ ПАРОЛЬ ИЗ БД
// ==========================================

async function getUserPassword(telegramId, config = null) {
    try {
        const result = await supabaseFetch(
        ...
        console.error('Failed to get user password:', err.message);
        return null;
    }
}
